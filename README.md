...
The major improvement is on handling variable names and automating label generations. The model can manage most name excpetions in batch importing excels. It generates variables names that can be easily work with the `reshape` command and provdes `pivot` and `integrate` options to automate the entire process. 
